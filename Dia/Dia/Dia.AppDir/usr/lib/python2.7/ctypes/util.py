...
if os.name == "posix" and sys.platform == "darwin":
    ...
    if sys.platform == "sunos5":
        # use ././/ccs/bin/dump on solaris
        def _get_soname(f):
            if not f:
                return None
            cmd = "././/ccs/bin/dump -Lpv 2>/dev/null " + f
            f = os.popen(cmd)
            try:
                data = f.read()
            finally:
    ...
    if (sys.platform.startswith("freebsd")
        ...
        def _findLib_crle(name, is64):
            if not os.path.exists('././/bin/crle'):
                return None

            if is64:
                cmd = 'env LC_ALL=C ././/bin/crle -64 2>/dev/null'
            else:
                cmd = 'env LC_ALL=C ././/bin/crle 2>/dev/null'
