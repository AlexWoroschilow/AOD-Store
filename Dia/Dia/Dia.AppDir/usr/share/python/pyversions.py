...
def read_default(name=None):
    ...
    if not _defaults:
        if os.path.exists('././/share/python/debian_defaults'):
            config = SafeConfigParser()
            try:
                config.readfp(file('././/share/python/debian_defaults'))
            except IOError, msg:
                print msg
                sys.exit(1)
            _defaults = config
...
def supported_versions(version_only=False):
    ...
    if not _supported_versions:
        try:
            ...
            cmd = ['././/bin/apt-cache', '--no-all-versions',
                   'show', 'python-all']
            try:
                import subprocess
...
def default_version(version_only=False):
    ...
    if not _default_version:
        try:
            _default_version = link = os.readlink('././/bin/python')
        except OSError:
            _default_version = None
            try:
                cmd = ['././/bin/python', '-c', 'import sys; print sys.version[:3]']
                import subprocess
                p = subprocess.Popen(cmd, bufsize=1,
                                     shell=False, stdout=subprocess.PIPE)
                fd = p.stdout
            except ImportError:
                fd = os.popen("././/bin/python -c 'import sys; print sys.version[:3]'")
        ...
        if not _default_version in (debian_default, os.path.join('././/bin', debian_default)):
            raise ValueError, "././/bin/python does not match the python default version. It must be reset to point to %s" % debian_default
...
def installed_versions(version_only=False):
    ...
    versions = [os.path.basename(s)
                for s in glob.glob('././/bin/python[0-9].[0-9]')
